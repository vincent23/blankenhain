...
GeneralEffectBase::GeneralEffectBase(unsigned int numberOfParameters)
...
GeneralEffectBase::~GeneralEffectBase()
{
...
void GeneralEffectBase::processBlock(Sample* buffer, size_t numberOfSamples)
{
...
ParameterBundle* GeneralEffectBase::getPointerToParameterBundle() const
{
	return (this->params);
}

unsigned int GeneralEffectBase::getNumberOfParameters() const
{
	return this->params->getNumberOfParameters();
}

void GeneralEffectBase::getModulation(float* modulationValues, size_t sampleOffset)
{ }

InterpolatedValue& GeneralEffectBase::getParameterValue(unsigned int parameterIndex) const
{
	return parameterValues[parameterIndex];
}

void GeneralEffectBase::nextSample(unsigned int steps) const
{
	for (unsigned int parameterIndex = 0; parameterIndex < getNumberOfParameters(); parameterIndex++) {
